...
namespace Free.Crypto.CRCChecksums
{
	...
	{
		/// <summary>
		/// Implements <see cref="ICRC"/> and <see cref="ICRC{T}"/> for a maximum 64-bit width CRC for unreflected input (MSBit first).
		/// </summary>
		/// <threadsafety static="false" instance="false"/>
		public class UnreflectedULong : ICRC<ulong>
		{
			const int BytesPreThread=4*1024; // IMPORTANT: Must be a power-of-two!

			int width;
			ulong register, init, xorOutput;
			ulong[] table;
			bool reflectOutput;
			ulong[] combineMatrix;
			...
			/// <param name="table">The table generated from the polynomial to use. Use <see cref="GenerateTable(ulong, int)"/> with the polynomial and its width to generate the table.</param>
			...
			public UnreflectedULong(ulong polynomial, ulong[] table, ulong init, bool refOut, ulong xorOut, int width=64)
			{
			...
			/// <param name="xorOut">Value to be XORed with the reflected or unreflected register depending on <paramref name="refOut"/> before output. Filled in the least significant bits.</param>
			/// <param name="width">The width of the polynomial in bits. Must be greater than 0 and less than 65. Default is 64.</param>
			public UnreflectedULong(ulong polynomial, ulong init, bool refOut, ulong xorOut, int width=64)
			...
			/// </summary>
			public ulong Value { get { return (reflectOutput?BitOrder.Reflect(register):register>>(64-width))^xorOutput; } }
			...
			public uint GetCRC() { return (uint)Value; }
			...
			{
				register=(register<<8)^table[((register>>56)^value)&0xFF];
			...
			public ICRC Add(byte[] data, int offset=0, int count=0)
			{
				...
				if(count<BytesPreThread)
				{
					...
					unsafe
					{
						fixed(byte* lDataFixed=data)
						{
							...
							do
							{
								lRegister=(lRegister<<8)^lTable[((lRegister>>56)^*lData++)&0xFF];
							}
							while(--lCount!=0);
						}
					}
				...
				ulong[] registers=new ulong[threads];
					...
					fixed(byte* lDataFixed=data)
					{
						...
						Parallel.For(0, threads,
							(n) =>
							{
								...
								do
								{
									lRegister=(lRegister<<8)^lTable[((lRegister>>56)^*i++)&0xFF];
								}
								while(--lCount!=0);

								registers[n]=lRegister>>(64-width);
							}
						);
					}
				}

				ulong reg=registers[0];
			...
			public ICRC Add(List<byte> data, int offset=0, int count=0)
			{
				...
				if(count<BytesPreThread)
				{
					...
					do
					{
						lRegister=(lRegister<<8)^lTable[((lRegister>>56)^lData[lOffset++])&0xFF];
					}
					while(--lCount!=0);
			...
			public ICRC Add(IEnumerable<byte> data)
			{
				...
				foreach(byte value in lData) lRegister=(lRegister<<8)^lTable[((lRegister>>56)^value)&0xFF];
		...
		/// <summary>
		/// Implements <see cref="ICRC"/> and <see cref="ICRC{T}"/> for a maximum 64-bit width CRC for reflected input (LSBit first).
		/// </summary>
		/// <threadsafety static="false" instance="false"/>
		public class ReflectedULong : ICRC<ulong>
		{
			const int BytesPreThread=4*1024; // IMPORTANT: Must be a power-of-two!
			...
			/// </summary>
			/// <param name="polynomial">The polynomial to use. Unreflected and filled in the least significant bits without the leading 1.</param>
			/// <param name="table">The table generated from the polynomial to use. Use <see cref="GenerateTableReflected(ulong, int)"/> with the polynomial and its width to generate the table.</param>
			/// <param name="init">The initial value of the register. Unreflected and filled in the least significant bits.</param>
			/// <param name="refOut">Set <b>true</b>, if register is to be reflected before XORing with <paramref name="xorOut"/> and output.</param>
			/// <param name="xorOut">Value to be XORed with the reflected or unreflected register depending on <paramref name="refOut"/> before output. Filled in the least significant bits.</param>
			/// <param name="width">The width of the polynomial in bits. Must be greater than 0 and less than 65. Default is 64.</param>
			public ReflectedULong(ulong polynomial, ulong[] table, ulong init, bool refOut, ulong xorOut, int width=64)
			{
				if(width<=0||width>64) throw new ArgumentOutOfRangeException("width", "Must be greater than 0 and less than 65.");
				...
				for(ulong n=1, row=1; (int)n<width; n++, row<<=1) mat1[n]=row;
			...
			/// <param name="xorOut">Value to be XORed with the reflected or unreflected register depending on <paramref name="refOut"/> before output. Filled in the least significant bits.</param>
			/// <param name="width">The width of the polynomial in bits. Must be greater than 0 and less than 65. Default is 64.</param>
			public ReflectedULong(ulong polynomial, ulong init, bool refOut, ulong xorOut, int width=64)
			...
			/// Gets the CRC value (not the register) for the message bytes processed so far.
			/// </summary>
			public ulong Value { get { return (reflectOutput?register:BitOrder.Reflect(register, width))^xorOutput; } }
