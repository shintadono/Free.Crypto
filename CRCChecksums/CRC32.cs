...
{
	/// <summary>
	/// Implements <see cref="ICRC"/> and <see cref="ICRC{T}"/> for THE 32-bit width CRC (aka Ethernet-CRC) and variations thereupon.
	/// </summary>
	/// <remarks>
	/// <para>"Rocksoft^tm Model CRC Algorithm" parameter:
	/// <list type="table">
	///  <listheader><term>Parameter</term><description>0x04C11DB7</description></listheader>
	///  <item><term>Name</term><description>Not needed.</description></item>
	///  <item><term>Width</term><description>32</description></item>
	///  <item><term>Poly</term><description>0x04C11DB7</description></item>
	///  <item><term>Init</term><description>Can be defined on instantiation. See <see cref="CRC32.CRC32"/>.</description></item>
	///  <item><term>RefIn</term><description>true</description></item>
	///  <item><term>RefOut</term><description>Can be defined on instantiation. See <see cref="CRC32.CRC32"/>.</description></item>
	///  <item><term>XorOut</term><description>Can be defined on instantiation. See <see cref="CRC32.CRC32"/>.</description></item>
	///  <item><term>Check</term><description>Depends on the other parameters.</description></item>
	/// </list></para>
	/// <para>
	/// This implementation utilizes a technique that has basically the same idea as Intel's "Slicing by Eight"-algorithm (SB8),
	/// which is just another table-driven CRC only with a few more tables. One could call this implementations algorithm "Slicing
	/// by Four". Thus it is just twice as fast as a regular table-driven CRC32 on single core machines, but not as fast as SB8,
	/// which gains another 50% performance boost on Intel's 64-bit platforms.
	/// </para>
	/// <para>
	/// Also this implementation utilizes the Task Parallel Library (TPL) if possible.
	/// </para>
	/// </remarks>
	/// <threadsafety static="false" instance="false"/>
	[CLSCompliant(false)]
	public class CRC32 : ICRC<uint>
	{
		const int BytesPreThread=4*1024; // IMPORTANT: Must be a power-of-two!
		const uint Polynomial=0x04C11DB7;

		#region public static uint[] crc_table0..3={...};
		/// <summary>
		/// Lookup-Table of CRC32 (AKA Ethernet-CRC).
		/// </summary>
		public static uint[] crc_table0=
			{
		...
		/// <summary>
		/// Lookup-Table of CRC32 (AKA Ethernet-CRC) with values padded with 1 zero (0).
		/// </summary>
		public static uint[] crc_table1=
			{
		...
		/// <summary>
		/// Lookup-Table of CRC32 (AKA Ethernet-CRC) with values padded with 2 zeros (0).
		/// </summary>
		public static uint[] crc_table2=
			{
		...
		/// <summary>
		/// Lookup-Table of CRC32 (AKA Ethernet-CRC) with values padded with 3 zeros (0).
		/// </summary>
		public static uint[] crc_table3=
			{
		...
		/// <summary>
		/// Creates an instance of a CRC algorithm with the behaviour according to the parameters. (polynomial=0x04C11DB7, refIn=true)
		/// </summary>
		...
		public CRC32(uint init=0xFFFFFFFF, bool refOut=true, uint xorOut=0xFFFFFFFF)
		{
			uint polynomial=BitOrder.Reflect(Polynomial);
			init=BitOrder.Reflect(init);
			...
			uint[] mat1=new uint[32], mat2=new uint[32]; // Create matrices (for inplace squaring operation)
			...
			for(uint n=1, row=1; n<32; n++, row<<=1) mat1[n]=row;
			...
			CRC.MatrixSquare(mat2, mat1);

			// Square again to create 4-bit-operation matrix
			CRC.MatrixSquare(mat1, mat2);
		...
		/// </summary>
		public uint Value { get { return (reflectOutput?register:BitOrder.Reflect(register))^xorOutput; } }
		...
		{
			register=(register>>8)^crc_table0[(register^value)&0xFF];
		...
		public ICRC Add(byte[] data, int offset=0, int count=0)
		{
			...
			if(count<BytesPreThread)
			{
				...
				if(BitConverterPlus.IsLittleEndian)
				{
					// local copies of values and references make the code much faster
					uint[] lTable0=crc_table0, lTable1=crc_table1, lTable2=crc_table2, lTable3=crc_table3;
					...
					unsafe
					{
						fixed(byte* lDataFixed=data)
						{
							...
							while(lCount!=0&&((int)lData&3)!=0) // align buf to 4 byte border
							{
								lRegister=(lRegister>>8)^lTable0[(lRegister^*lData++)&0xFF];
								lCount--;
							}

							uint* lData4=(uint*)lData;
							while(lCount>=4)
							{
								lRegister^=*lData4++;
								lRegister=lTable3[lRegister&0xFF]^lTable2[(lRegister>>8)&0xFF]^lTable1[(lRegister>>16)&0xFF]^lTable0[lRegister>>24];
								lCount-=4;
							}
							lData=(byte*)lData4;

							if(lCount!=0) // anything missing?
							{
								do
								{
									lRegister=(lRegister>>8)^lTable0[(lRegister^*lData++)&0xFF];
					...
					if(BitConverterPlus.IsLittleEndian)
					{
						Parallel.For(0, threads,
							(n) =>
							{
								...
								while(lCount!=0&&((int)i&3)!=0) // align buf to 4 byte border
								{
									lRegister=(lRegister>>8)^lTable0[(lRegister^*i++)&0xFF];
									lCount--;
								}
			...
			{
				reg=CRC.MatrixMult(combineMatrix, reg);
				reg^=registers[i];
			}
		...
		public ICRC Add(IEnumerable<byte> data)
		{
			...
			foreach(byte value in lData) lRegister=(lRegister>>8)^lTable[(lRegister^value)&0xFF];
