...
namespace Free.Crypto.CRCChecksums.RocksoftTMModelCRCAlgorithms
{
	...
	public static class TABLE
	{
		...
		public static byte Get(byte[] table, IList<byte> data, int offset=0, int count=0)
		{
			...
			while(offset<endOfByteStream) register=(byte)(data[offset++]^table[register]);

			// Augmented part of the message
			for(int i=0; i<sizeof(byte); i++) register=table[register];
		...
		public static ushort Get(ushort[] table, IList<byte> data, int offset=0, int count=0)
		{
			...
			// Real part of the message
			while(offset<endOfByteStream) register=(ushort)(((register<<8)|data[offset++])^table[(register>>8)&0xFF]);

			// Augmented part of the message
			for(int i=0; i<sizeof(ushort); i++) register=(ushort)((register<<8)^table[(register>>8)&0xFF]);
		...
		public static uint Get(uint[] table, IList<byte> data, int offset=0, int count=0)
		{
			...
			// Real part of the message
			while(offset<endOfByteStream) register=((register<<8)|data[offset++])^table[(register>>24)&0xFF];

			// Augmented part of the message
			for(int i=0; i<sizeof(uint); i++) register=(register<<8)^table[(register>>24)&0xFF];
		...
		public static ulong Get(ulong[] table, IList<byte> data, int offset=0, int count=0)
		{
			...
			// Real part of the message
			while(offset<endOfByteStream) register=((register<<8)|data[offset++])^table[(register>>56)&0xFF];

			// Augmented part of the message
			for(int i=0; i<sizeof(ulong); i++) register=(register<<8)^table[(register>>56)&0xFF];
