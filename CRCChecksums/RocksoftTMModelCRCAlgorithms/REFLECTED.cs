...
namespace Free.Crypto.CRCChecksums.RocksoftTMModelCRCAlgorithms
{
	...
	public static class REFLECTED
	{
		...
		{
			if(reflectedTable==null) throw new ArgumentNullException("reflectedTable");
			if(reflectedTable.Length<256) throw new ArgumentOutOfRangeException("reflectedTable", "Must have at least 256 elements.");
			...
			while(offset<endOfByteStream) register=reflectedTable[(register^data[offset++])&0xFF];
		...
		public static ushort Get(ushort[] reflectedTable, IList<byte> data, int offset=0, int count=0)
		{
			...
			while(offset<endOfByteStream) register=(ushort)((register>>8)^reflectedTable[(register^data[offset++])&0xFF]);
		...
		public static uint Get(uint[] reflectedTable, IList<byte> data, int offset=0, int count=0)
		{
			...
			while(offset<endOfByteStream) register=(register>>8)^reflectedTable[(register^data[offset++])&0xFF];
